...
public class Event implements org.bukkit.event.Listener {
  ...
  public void onWeatherChange(
    ...
    if (event.toWeatherState()) {
      org.bukkit.Bukkit.broadcastMessage(
        "§b§lIl pleut, faite /rain pour arrêter la pluie."
      );
    }
  }
  ...
  public void onPlayerJoin(org.bukkit.event.player.PlayerJoinEvent event) {
    ...
    event.setJoinMessage(
      "§7[§2+§7] §f" +
      player.getName() +
      plugin.getConfig().getString("JoinMessage")
    );
  ...
  public void onPlayerQuit(org.bukkit.event.player.PlayerQuitEvent event) {
    ...
    event.setQuitMessage(
      "§7[§4-§7] §f" +
      player.getName() +
      plugin.getConfig().getString("LeftMessage")
    );
  ...
  public void onPlayerCommandPreprocess(
    ...
    if (
      ...
      command.startsWith("/bukkit:version") ||
      command.startsWith("/bukkit:about") ||
      command.startsWith("/me") ||
      command.startsWith("/trigger") ||
      command.startsWith("/minecraft:me") ||
      command.startsWith("/minecraft:trigger") ||
      command.startsWith("/give") ||
      command.startsWith("/minecraft:give") ||
      command.startsWith("/effect") ||
      command.startsWith("/minecraft:effect")
    ) {
      event.setCancelled(true);
      player.sendMessage(
        "[Bettersurvival] §cVous ne pouvez pas utiliser cette commande."
      );
    }
  }
  ...
  @EventHandler
  public void onPlayerCommandPreprocessMessage(
    org.bukkit.event.player.PlayerCommandPreprocessEvent event
  ) {
    org.bukkit.entity.Player player = event.getPlayer();
    String command = event.getMessage().toLowerCase();
    if (
      ...
      command.startsWith("/minecraft:tell") ||
      command.startsWith("/minecraft:msg") ||
      command.startsWith("/minecraft:w") ||
      command.startsWith("/minecraft:whisper")
    ) {
      event.setCancelled(true);
        ...
        if (target != null) {
          ...
          player
            ...
            .sendMessage(
              ChatMessageType.ACTION_BAR,
              TextComponent.fromLegacyText(
                "Vous avez reçu un message de " + target.getName()
              )
            );
        } else {
          player.sendMessage(
            "[Bettersurvival] §cLe joueur n'est pas connecté."
          );
        }
      } else {
        player.sendMessage(
          "[Bettersurvival] §cUtilisation: /tell <joueur> <message>"
        );
      }
    }
  }
  ...
  public void onEnderDragonDeath(
    ...
    if (entity instanceof org.bukkit.entity.EnderDragon) {
      ...
      if (killer != null) {
        org.bukkit.Bukkit.broadcastMessage(
          "§1" + killer.getName() + "§f a tué l'§5EnderDragon§f !"
        );
      } else {
  ...
  public void onWitherDeath(org.bukkit.event.entity.EntityDeathEvent event) {
    ...
    if (entity instanceof org.bukkit.entity.Wither) {
      ...
      if (killer != null) {
        org.bukkit.Bukkit.broadcastMessage(
          "§1" + killer.getName() + "§f a tué le §0Wither§f !"
        );
      } else {
